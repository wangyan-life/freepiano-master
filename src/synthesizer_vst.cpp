...
static LRESULT CALLBACK vst_editor_wndproc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  switch (uMsg) {
   ...
   case WM_TIMER: {
     AEffect *effect = (AEffect *)GetWindowLongPtr(hWnd, GWLP_USERDATA);
...
static HWND create_effect_window(AEffect *effect) {
  ...
  if (!initialized) {
    ...
    wc.lpszClassName = "DevPianoVstEffect";
    wc.hIconSm = NULL;
  ...
  // create window
  HWND hwnd = CreateWindow("DevPianoVstEffect", effectName, WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU, CW_USEDEFAULT, CW_USEDEFAULT, 0, 0, NULL, NULL, GetModuleHandle(NULL), NULL);
