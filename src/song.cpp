...
int song_open_lyt(const char *filename) {
  ...
  try {
    ...
    // read magic
    read(header, 16, fp);
    if (strcmp(header, "iDreamDevPianoSong") != 0)
      throw -1;
...
int song_open(const char *filename) {
  ...
  try {
    ...
    char magic[sizeof("DevPianoSong")];
    char instrument[256];

    // magic
    read(magic, sizeof("DevPianoSong"), fp);
    if (memcmp(magic, "DevPianoSong", sizeof("DevPianoSong")) != 0)
      throw -1;
...
int song_save(const char *filename) {
  ...
  if (song_end) {
    ...
    try {
      // magic
      write("DevPianoSong", sizeof("DevPianoSong"), fp);
